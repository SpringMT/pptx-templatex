...
ネイティブ表（table）内のセルもテキストボックスと同様に置換されます
（`replace_texts` 経由・リッチテキスト対応）。

行数可変のテーブルは、表の中に**テンプレート行**を 1 行だけ置き、セルに
`{{ <key>.<field> }}` を書いておきます。`replace_table_rows` で行データを渡すと、
テンプレート行が行数分複製され（罫線・塗り・フォントを引き継ぐ）、
テンプレート行自体は削除されます:
  ...
  "slides": [
    {
      ...
      "replace_table_rows": {
        "rows": [
          {"c1": "台風接近", "c2": "行程を前倒し", "c3": "全行程を実施"},
          {"c1": "語り部欠席", "c2": "映像教材に切替", "c3": "予定どおり終了"}
        ]
      }
    }
  ]
